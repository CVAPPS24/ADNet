...
class MHSA_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        #(B,C,D,W,H)---> (B,H,C//H,D*H*W)---> (B,H,D*H*W,D*H*W)
        content_content = torch.matmul(q.permute(0, 1, 3, 2), k)
        #(B,H,C//H,D,W,H)-->(B,H,C//H,D*W*H)-->(B,H,D*W*H,C//H)
        content_position = (self.rel_h + self.rel_w + self.rel_d).view(1, self.heads, C // self.heads, -1).permute(0, 1, 3, 2)
        #(B,H,D*W*H,C//H)-->(B,H,D*W*H,C//H)-->(B,H,D*H*W,D*H*W)
...
class Temp2(nn.Module):
    def __init__(self, block, layers, heads,resolution,mhsa, shortcut_type='B'):
        ...
        self.layer5 = self._make_layer(
            block, 512, layers[4], shortcut_type, stride=1, dilation=2,heads=heads,mhsa=mhsa)
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch_1 = Temp2(block,layers,heads,self.resolution,mhsa=mhsa).to(self.no_cuda) #cuda:1
        self.branch_2 = Temp2(block,layers,heads,self.resolution2,mhsa=mhsa).to(self.no_cuda) #cuda:1
...
def resnet18(num_classes=3,no_cuda="cuda:1", resolution=np.array([160,160,160]), heads=4,**kwargs):
    """Constructs a ResNet-18 model.
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2, 2],no_cuda=no_cuda, num_seg_classes=num_classes, resolution=resolution, heads=heads,mhsa=True, **kwargs)
    return model
...
def test():
    x = torch.randn([4, 1, 160, 160,160])#.cuda(0)
    model = resnet18(no_cuda="cpu",resolution=np.array(x.shape[2:]), heads=4) #tuple(x.shape[2:])
    # print(model(x)[0].size(),model(x)[1].size())
